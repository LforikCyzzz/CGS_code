...
# # Large
# from CGS import DepthWiseSharing_Large, PointWiseSharing_Large_1, PointWiseSharing_Large_2

# # Small
# from CGS import DepthWiseSharing_Small, PointWiseSharing_Small_1, PointWiseSharing_Small_2

# Base
from CGS import DepthWiseSharing_Base, PointWiseSharing_Base_1, PointWiseSharing_Base_2
...
def get_conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias):
    ...
    if(True == use_large_impl):
        print(in_channels, out_channels, kernel_size)
        return DepthWiseSharing_Base(in_channels = in_channels, kernel_size = kernel_size, 
...
class RepLKBlock(nn.Module):
    ...
    def __init__(self, in_channels, dw_channels, block_lk_size, small_kernel, drop_path, small_kernel_merged=False):
        ...
        self.lk_nonlinear = nn.LeakyReLU()
...
class RepLKBlock_sharing_v2(nn.Module):
    def __init__(self, channels, dw_channels, block_lk_size, small_kernel, drop_path, small_kernel_merged=False, R=200):
        super().__init__()
        self.block_pw1 = PointWiseSharing_Base_1(channels, dw_channels, 1, 1, 0, groups=1, R=R)
        self.block_pw2 = PointWiseSharing_Base_2(dw_channels, channels, 1, 1, 0, groups=1, R=R)

        self.large_kernel = ReparamLargeKernelConv(in_channels=dw_channels, out_channels=dw_channels, kernel_size=block_lk_size,
                                                  stride=1, groups=dw_channels, small_kernel=small_kernel, 
                                                  small_kernel_merged=small_kernel_merged)
        self.lk_nonlinear = nn.ReLU()
        self.pw1_nonlinear = nn.LeakyReLU() 
        self.prelkb_bn1 = get_bn(channels)
        self.prelkb_bn2 = get_bn(dw_channels)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()

    def forward(self, x):
        out = self.prelkb_bn1(x)
        out = self.block_pw1(out)
        out = self.pw1_nonlinear(out)

        out = self.large_kernel(out)
        out = self.lk_nonlinear(out)

        out = self.prelkb_bn2(out)
        out = self.block_pw2(out)

        return x + self.drop_path(out)


class CGSStage(nn.Module):
...
class CGSStage_Sharing(nn.Module):
    def __init__(self, channels, num_blocks, stage_lk_size, drop_path,
                 small_kernel, dw_ratio=8, 
                 use_checkpoint=False,      # train with torch.utils.checkpoint to save memory
                 small_kernel_merged=False,
                 norm_intermediate_features=False, R=200):
        super().__init__()
        self.use_checkpoint = use_checkpoint
        ...
        for i in range(num_blocks):
            block_drop_path = drop_path[i] if isinstance(drop_path, list) else drop_path
            #   Assume all RepLK Blocks within a stage share the same lk_size. You may tune it on your own model.
            replk_block = RepLKBlock_sharing_v2(channels=channels, dw_channels=dw_channels, block_lk_size=stage_lk_size,
                                     small_kernel=small_kernel, drop_path=block_drop_path, small_kernel_merged=small_kernel_merged, R=R)
            blks.append(replk_block)
...
class RepLKNet_CGS(nn.Module):

    def __init__(self, large_kernel_sizes, layers, channels, sharings, drop_path_rate, small_kernel,
                 dw_ratio=1, dw2_ratio=4, ffn_ratio=4, in_channels=3, num_classes=1000, out_indices=None,
        ...
        for stage_idx in range(self.num_stages):
            layer = None
            if(True == sharings[stage_idx]):
                R = 200
                layer = CGSStage_Sharing(channels=channels[stage_idx], num_blocks=layers[stage_idx],
                                            stage_lk_size=large_kernel_sizes[stage_idx],
                                            drop_path=dpr[sum(layers[:stage_idx]):sum(layers[:stage_idx + 1])],
                                            small_kernel=small_kernel, dw_ratio=dw2_ratio,
                                            use_checkpoint=use_checkpoint, small_kernel_merged=small_kernel_merged,
                                            norm_intermediate_features=norm_intermediate_features, R=R)
            else:
                layer = CGSStage(channels=channels[stage_idx], num_blocks=layers[stage_idx],
                                    stage_lk_size=large_kernel_sizes[stage_idx],
                                    drop_path=dpr[sum(layers[:stage_idx]):sum(layers[:stage_idx + 1])],
    ...
    def deep_fuse_BN(self):
        for m in self.modules():
            ...
            if hasattr(m[0], 'kernel_size') and hasattr(m[0], 'weight') and isinstance(m[1], nn.BatchNorm2d):
                ...
                fused_conv = get_conv2d(conv.in_channels, conv.out_channels, kernel_size=conv.kernel_size, stride=conv.stride,
...
def create_RepLKNet_CGS(drop_path_rate=0.3, num_classes=1000, use_checkpoint=True, small_kernel_merged=False):
    # large base small
    return RepLKNet_CGS(large_kernel_sizes=[31,29,27,13], layers=[2,2,18,2], channels=[128,256,512,1024], sharings=[False, False, True, True],
